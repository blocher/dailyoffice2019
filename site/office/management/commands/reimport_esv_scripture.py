    ...
    def handle(self, *args, **options):
        ...
        for scripture in scriptures:
            ...
            if not passage or passage.strip() == '':
                self.stdout.write(
                    self.style.WARNING(f'[{total_count}] Skipping empty passage')
                )
                skipped_count += 1
                continue
            ...
            try:
                ...
                if not html or html.strip() == '':
                    self.stdout.write(
                        self.style.WARNING(f'  → Empty HTML returned for {passage}')
                    )
                    skipped_count += 1
                    continue
        ...
        if error_count > 0:
            self.stdout.write(self.style.ERROR(
                f'Command completed with {error_count} error(s). Please review the errors above.'
            ))
        else:
