...
def get_church_years():
    ...
    if settings.MODE == "app":
        for year in range(int(settings.FIRST_BEGINNING_YEAR_APP[0]), int(settings.LAST_BEGINNING_YEAR_APP[0]) + 1):
            yield {"start_year": year, "end_year": year + 1}
    else:
        for year in range(int(settings.FIRST_BEGINNING_YEAR[0]), int(settings.LAST_BEGINNING_YEAR[0]) + 1):
            yield {"start_year": year, "end_year": year + 1}
